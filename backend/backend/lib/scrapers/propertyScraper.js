...
import { scrapeWithFirecrawl } from './firecrawlScraper.js';
...
export async function scrapeProperty(url) {
  try {
    ...
    // Try browser scraping first, fallback to Firecrawl
    let propertyData;
    try {
      console.log('🔄 Attempting browser scraping...');
      propertyData = await browserScrape(url);
      console.log('✅ Browser scraping succeeded');
    } catch (browserError) {
      console.log('⚠️ Browser scraping failed, trying Firecrawl fallback:', browserError.message);
      try {
